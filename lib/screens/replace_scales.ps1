$file = "d:\noor_rewards-main\noor_rewards-main\lib\screens\dhikr_screen.dart"
$lines = Get-Content -Path $file
$start = -1
$end = -1
for ($i=0; $i -lt $lines.Length; $i++) {
    if ($lines[$i] -match "?? Unparalleled Scales") { $start = $i }
    if ($lines[$i] -match "?? Sunrise Glory") { $end = $i; break }
}

if ($start -gt 0 -and $end -gt $start) {
    $newClassContent = @"
// =============================================================================
// ?? Released Slaves (????? ????) — 10 slaves freed
// =============================================================================
...
class _UnparalleledScalesState extends State<_UnparalleledScales> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (_, __) => SizedBox(height: 290, child: CustomPaint(painter: _UnparalleledScalesPainter(progress: _grow.value, pulse: _pulse.value, starPhase: _starCtrl.value, particlePhase: _pAnim.value, particles: _particles, isComplete: widget.isComplete, pointsToday: widget.pointsToday, punchScale: _punch.value, shockPhase: _shock.value))),
    );
  }
}
...
class _UnparalleledScalesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (pointsToday > 0) {
      ...
      final badgeW = tp3.width + 36; final badgeH = tp3.height + 18;
  ...
  void _drawFreedSouls(Canvas canvas, double cx, double cy, double w) {
    for (int i = 0; i < 10; i++) {
        ...
        final dist = maxDist * math.sin(flyP * math.pi / 2); // ease out
  ...
  @override
  bool shouldRepaint(_UnparalleledScalesPainter o) => o.progress != progress || o.pulse != pulse || o.starPhase != starPhase || o.particlePhase != particlePhase || o.isComplete != isComplete || o.pointsToday != pointsToday || o.punchScale != punchScale || o.shockPhase != shockPhase;
}
"@

    $before = $lines[0..($start - 2)]
    $after = $lines[$end..($lines.Length - 1)]
    $finalContent = $before -join "`r`n"
    $finalContent += "`r`n" + $newClassContent + "`r`n// =============================================================================`r`n"
    $finalContent += $after -join "`r`n"

    Set-Content -Path $file -Value $finalContent
    Write-Output "Successfully replaced UnparalleledScales with FreedSlaves cage animation."
} else {
    Write-Output "Could not find start/end lines! Start:$start End:$end"
}
