...
void _writeBatch(String num, int i, int total, List<MapEntry<String, String>> slice) {
  ...
  buf.writeln();
  buf.writeln('Return **ONE single JSON object** inside **ONE** fenced code block. Do NOT split into multiple code blocks. The top-level object has exactly 7 keys — the locale codes `ur`, `ar`, `fr`, `id`, `ms`, `ru`, `tr` — in this order. Each value is a flat `{key: translation}` object with the SAME keys as the source below.');
  buf.writeln();
  buf.writeln('No preamble, no closing text, no comments, no explanation — just the one fenced JSON block.');
  buf.writeln();
