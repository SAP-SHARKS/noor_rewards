...
void main() {
  ...
  for (int i = 0; i < lines.length; i++) {
    if (lines[i].contains('class _UnparalleledScales extends StatefulWidget')) {
      // Find the comment line before it
      startIdx = i - 1;
      while (startIdx > 0 && lines[startIdx].trim().startsWith('//')) {
         startIdx--;
      }
      startIdx++; // include the comment
    }
    if (lines[i].contains('class _SunriseGlory extends StatefulWidget')) {
      endIdx = i - 1;
  ...
  if (startIdx != -1 && endIdx != -1 && startIdx < endIdx) {
    ...
    final after = lines.sublist(endIdx).join("\n");
...
class _FreedSlavesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < starPos.length; i++) {
        final tw = 0.5 + 0.5 * math.sin(starPhase * 3.14159 * 2 + i * 0.8);
    ...
    if (particlePhase > 0 && particlePhase < 1) {
        for (final p in particles) {
            ...
            final angle = p.x * 3.14159 * 2; final dist = 15 + t * w * 0.28;
  ...
  void _drawCage(Canvas canvas, double cx, double cy, double w) {
    ...
    final doorAngle = -3.14159 * 0.75 * doorOpenPhase;
  ...
  void _drawFreedSouls(Canvas canvas, double cx, double cy, double w) {
    for (int i = 0; i < 10; i++) {
        ...
        final dist = maxDist * (math.sin(flyP * 3.14159 / 2));

        final floatWobbleX = math.sin(starPhase * 3.14159 * 2 + i) * 6.0;
        final floatWobbleY = math.cos(starPhase * 3.14159 * 2 + i) * 4.0;
        ...
        final flap = math.sin(starPhase * 3.14159 * 12 + i * 1.5) * 6.0 + 2.0; 
...
""";
    ...
    // Create new content string adding back the double newline separation
    ...
    print('Could not find Unparalleled Scales. startIdx = $startIdx, endIdx = $endIdx');
  }
}
